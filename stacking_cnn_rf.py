...
NUM_CLASSES = 1  # Agora é uma classificação binária com uma saída
...
# Extrair as classes reais do test_generator
...
# Extraindo características com o CNN para usar no RandomForest
cnn_feature_extractor = Sequential(cnn_model.layers[:-1])  # Remove última camada de saída para usar como extrator
train_features = cnn_feature_extractor.predict(train_generator)
train_features = train_features.reshape(train_features.shape[0], -1)  # Ajustar o formato

test_features = cnn_feature_extractor.predict(test_generator)
test_features = test_features.reshape(test_features.shape[0], -1)  # Ajustar o formato

# Agora, usando os recursos extraídos como entrada para o RandomForest
rf_classifier = RandomForestClassifier(n_estimators=200)
rf_classifier.fit(train_features, train_generator.classes)

# Previsões do RandomForest
rf_predictions = rf_classifier.predict(test_features)

# Empilhamento: Usamos LogisticRegression como meta-modelo
meta_model = LogisticRegression()
meta_model.fit(np.column_stack((cnn_predictions.flatten(), rf_predictions)), true_classes)

# Previsões finais com stacking
final_predictions = meta_model.predict(np.column_stack((cnn_predictions.flatten(), rf_predictions)))

# Calcular a acurácia do modelo stacking
stacking_acc = accuracy_score(true_classes, final_predictions)
print(f"Acurácia do modelo stacking no conjunto de teste: {stacking_acc * 100:.2f}%")

# Calculando outras métricas com zero_division=0
...
class_report = classification_report(true_classes, final_predictions, zero_division=0)
