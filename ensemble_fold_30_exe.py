...
OUTPUT_CSV = "ensemble_average_results.csv"
...
for execution in range(EXECUTIONS):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(data)):
        ...
        print(
            f"Fold {fold + 1} concluído: Acurácia={stacking_acc:.2f}, Precisão={precision:.2f}, Recall={recall:.2f}, F1-Score={f1:.2f}")

        # Salvar métricas para a execução
        execution_metrics['Accuracy'].append(stacking_acc)
    ...
    print(
        f"Médias da execução {execution + 1}: Acurácia={avg_accuracy:.2f}, Precisão={avg_precision:.2f}, Recall={avg_recall:.2f}, F1-Score={avg_f1:.2f}")
...
with open(OUTPUT_CSV, mode='w', newline='') as file:
    writer = csv.DictWriter(file,
                            fieldnames=['Execution', 'Avg_Accuracy', 'Avg_Precision', 'Avg_Recall', 'Avg_F1_Score'])
    writer.writeheader()
    writer.writerows(average_results)

print("Treinamento concluído e médias salvas em average_results.csv.")
